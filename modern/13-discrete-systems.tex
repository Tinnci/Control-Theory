...
为无失真恢复连续信号，采样频率必须：
\begin{align*}
f_s \geq 2f_{\max} \quad \text{或采样周期：} \quad T \leq \frac{\pi}{\omega_{\max}}
\end{align*}

其中 $f_{\max}$ 是信号的最高频率分量。

\textbf{工程实践：}一般取 $f_s = (6 \sim 10) f_{\max}$，或采样周期 $T = (0.1 \sim 0.5) \tau$（$\tau$ 为系统时间常数）

\vspace{0.5cm}
\textbf{频谱混叠（Aliasing）警告：}

当采样频率不满足采样定理时，高频信号会\textquotedblleft 伪装\textquotedblright 成低频信号，导致\textbf{不可恢复}的信息丢失。

\textbf{例子：}信号频率$f=9$Hz，采样频率$f_s=10$Hz（违反定理）。采样后看到的频率是$|f-f_s|=1$Hz——高频信号被误认为低频！
...
% 原始高频信号
\draw[blue, thick, domain=0:3, samples=100] plot (\x, {0.8*sin(360*3*\x)});
% 采样点
\foreach \x in {0, 0.3, 0.6, 0.9, 1.2, 1.5, 1.8, 2.1, 2.4, 2.7, 3.0} {
    \fill[red] (\x, {0.8*sin(360*3*\x)}) circle (1.5pt);
}
% 采样后看到的低频信号
\draw[red, dashed, thick, domain=0:3, samples=50] plot (\x, {0.8*sin(360*0.3*\x)});

\draw[->] (0,-1.2) -- (3.2,-1.2) node[right] {$t$};
\draw (0,-1.2) -- (0,-1.3) node[below] {$0$};
\draw (3,-1.2) -- (3,-1.3) node[below] {$3$s};

\node[blue, above left] at (0.5, 0.8) {\small 原始$f=9$Hz};
\node[red, below left] at (0.5, -0.8) {\small 采样后看到$f=1$Hz};
\end{tikzpicture}
\end{center}

\textbf{结论}：采样定理不是\textquotedblleft 理论建议\textquotedblright，而是\textbf{物理约束}——违反它，控制系统会看到错误的世界！

\subsubsection{数字控制系统的基本结构}

\textbf{核心问题}：计算机无法输出\textquotedblleft 真正的连续信号\textquotedblright——它只能在每个采样周期\textquotedblleft 刷新\textquotedblright 一次输出值并保持。

\begin{center}
\begin{tikzpicture}[scale=1.0, node distance=2cm, >=stealth]
% 节点定义
\node[draw, rectangle, minimum width=1.8cm, minimum height=1cm] (sampler) {采样器};
\node[draw, rectangle, minimum width=2.2cm, minimum height=1cm, right=of sampler] (controller) {数字控制器};
\node[draw, rectangle, minimum width=1.8cm, minimum height=1cm, right=of controller] (zoh) {ZOH};
\node[draw, rectangle, minimum width=2.2cm, minimum height=1cm, right=of zoh] (plant) {被控对象};

% 连接线
\draw[->] (sampler) -- node[above] {\small $e(kT)$} (controller);
\draw[->] (controller) -- node[above] {\small $u(kT)$} (zoh);
\draw[->] (zoh) -- node[above] {\small $u(t)$} (plant);
\draw[->] (plant) -- ++(2,0) node[right] {$y(t)$} -- ++(0,-1.5) -| (sampler);
\draw[<-] (sampler) -- ++(-2,0) node[left] {$r(t)$};

% 信号波形示意
\node[below=1.5cm of controller, align=center] (pulse) {
    \begin{tikzpicture}[scale=0.35]
        \draw[->] (0,0) -- (5,0) node[right] {\tiny $t$};
        \draw[->] (0,0) -- (0,2);
        \foreach \x in {1,2,3,4} {
            \draw[red, thick, ->] (\x, 0) -- (\x, {0.5+0.3*\x});
        }
        \node[below] at (2.5, -0.5) {\small 脉冲序列};
    \end{tikzpicture}
};

\node[below=1.5cm of zoh, align=center] (staircase) {
    \begin{tikzpicture}[scale=0.35]
        \draw[->] (0,0) -- (5,0) node[right] {\tiny $t$};
        \draw[->] (0,0) -- (0,2);
        \draw[blue, thick] (0,0.5) -- (1,0.5) -- (1,0.8) -- (2,0.8) -- (2,1.1) -- (3,1.1) -- (3,1.4) -- (4,1.4) -- (4,1.7) -- (5,1.7);
        \node[below] at (2.5, -0.5) {\small 阶梯波};
    \end{tikzpicture}
};

\draw[->, dashed] (controller.south) -- (pulse.north);
\draw[->, dashed] (zoh.south) -- (staircase.north);
\end{tikzpicture}
\end{center}

\textbf{零阶保持器（ZOH）的关键作用：}

\begin{enumerate}
    \item \textbf{物理意义}：ZOH是\textbf{连接数字世界和物理世界的桥梁}
    \begin{itemize}
        \item 数字控制器输出：一串数字 $u(0), u(T), u(2T), \ldots$（离散脉冲）
        \item 被控对象需要：连续信号 $u(t)$（物理世界没有\textquotedblleft 脉冲\textquotedblright ）
        \item ZOH的工作：在每个周期$[kT, (k+1)T)$内保持$u(kT)$不变
    \end{itemize}
    
    \item \textbf{数学模型}：ZOH不是\textquotedblleft 透明\textquotedblright 的！它有自己的传递函数
    \begin{align*}
    G_h(s) = \frac{1-e^{-Ts}}{s}
    \end{align*}
    
    \item \textbf{关键洞察}：ZOH引入相位滞后，\textbf{改变系统动态特性}
    \begin{itemize}
        \item 这就是为什么离散系统不是\textquotedblleft 连续系统简单采样\textquotedblright 
        \item 计算离散模型时\textbf{必须}包含ZOH（否则模型完全错误）
    \end{itemize}
\end{enumerate}

\textbf{贯穿全章的示例系统：}

考虑一个简单的一阶连续系统（RC电路、温度控制等）：
\begin{align*}
G_p(s) = \frac{1}{s+1}
\end{align*}

时间常数$\tau = 1$秒。选择采样周期$T=0.1$秒（$T=0.1\tau$，符合工程实践）。

\textbf{问题}：这个系统在离散化后会变成什么样子？稳定性如何？如何设计控制器？
...
\subsection{离散系统的建模与分析}

\textbf{核心问题}：如何像分析连续系统一样分析离散系统？

连续系统分析的\textquotedblleft 三部曲\textquotedblright：
\begin{enumerate}
    \item \textbf{建模}：获得传递函数$G(s)$（系统输入输出关系）
    \item \textbf{稳定性}：判断极点位置（左半平面 vs 右半平面）
    \item \textbf{性能}：计算瞬态（超调、调节时间）和稳态（误差）指标
\end{enumerate}

离散系统完全类似，但工具和判据不同：
\begin{enumerate}
    \item \textbf{建模}：脉冲传递函数$G(z)$（必须考虑ZOH！）
    \item \textbf{稳定性}：极点在单位圆内（不是左半平面）
    \item \textbf{性能}：Z反变换+终值定理
\end{enumerate}

\textbf{继续我们的示例}：$G_p(s) = \frac{1}{s+1}$，$T=0.1$s

\subsubsection{脉冲传递函数与零阶保持器}

\textbf{核心步骤}：从连续模型$G_p(s)$到离散模型$G(z)$
...
对于 $G(s) = \frac{1}{s+a}$：
\begin{align*}
G(z) &= (1-z^{-1})\mathcal{Z}\left\{\frac{1}{s(s+a)}\right\} \\
&= \frac{1-e^{-aT}}{z-e^{-aT}}
\end{align*}

对于 $G(s) = \frac{1}{s(s+a)}$：
\begin{align*}
G(z) &= (1-z^{-1})\mathcal{Z}\left\{\frac{1}{s^2(s+a)}\right\} \\
&= \frac{T - \frac{1-e^{-aT}}{a}}{(z-1)(z-e^{-aT})}
\end{align*}
\end{minipage}\hfill
...
\vspace{0.5cm}
\textbf{【贯穿例子】我们的示例系统：}$G_p(s) = \frac{1}{s+1}$，$T=0.1$s

使用公式（$a=1$）：
\begin{align*}
G(z) &= \frac{1-e^{-aT}}{z-e^{-aT}} = \frac{1-e^{-0.1}}{z-e^{-0.1}} \\
&= \frac{0.0952}{z-0.9048} \quad \text{（这是离散化后的系统模型）}
\end{align*}

\textbf{观察}：
\begin{itemize}
    \item 连续系统极点：$s=-1$（左半平面，稳定）
    \item 离散系统极点：$z=e^{-0.1} = 0.9048$（$|z|=0.9048 < 1$，单位圆内，稳定）
    \item 映射关系验证：$z = e^{sT} = e^{-1 \times 0.1} = 0.9048$ ✓
\end{itemize}
...
\textbf{多种判定方法：}
\begin{enumerate}
    \item \textbf{特征值法}：直接求特征值，判断模是否小于1（最简单，首选）
    \item \textbf{劳斯判据}：通过双线性变换转换到w平面（下文详述，适用于多项式形式）
    \item \textbf{Jury判据}：直接在z域判断（下文详述，直接但较繁琐）
    \item \textbf{李雅普诺夫方法}：能量递减法，适用于状态空间形式
    \begin{itemize}
        \item 理论基础见第\ref{sec:lyapunov-stability}章第\ref{subsec:discrete-lyapunov}节
        \item 包含离散李雅普诺夫方程 $A^TPA-P=-Q$ 的详细推导
        \item 提供完整的3×3矩阵稳定性分析例题
        \item 给出特征值法与李雅普诺夫法的对比
    \end{itemize}
\end{enumerate}

\textbf{重要提醒：}
\begin{itemize}
    \item 离散系统的稳定域是\textbf{单位圆内}（$|z|<1$），与连续系统的左半平面（$\text{Re}(s)<0$）完全不同！
    \item \textbf{实际工程推荐}：对于状态空间形式，优先用特征值法（MATLAB/Python一行代码）
    \item \textbf{理论分析推荐}：需要深入理解或鲁棒性分析时，使用李雅普诺夫方法（详见第\ref{subsec:discrete-lyapunov}节）
    \item 本节后续的劳斯/Jury判据主要用于传递函数形式的快速判断
\end{itemize}
...
假设单位负反馈，闭环传递函数：
\begin{align*}
\Phi(z) = \frac{G(z)}{1+G(z)} = \frac{0.0952/(z-0.9048)}{1+0.0952/(z-0.9048)} = \frac{0.0952}{z-0.81}
\end{align*}

闭环极点：$z=0.81$，$|z|=0.81 < 1$ $\implies$ \textbf{稳定} ✓
...
\textbf{【贯穿例子】我们的系统阶跃响应：}

单位阶跃输入$R(z) = \frac{z}{z-1}$，闭环输出：
\begin{align*}
Y(z) = \Phi(z)R(z) = \frac{0.0952}{z-0.81} \cdot \frac{z}{z-1} = \frac{0.0952z}{(z-0.81)(z-1)}
\end{align*}

部分分式：$Y(z) = \frac{z}{z-1} - \frac{0.5z}{z-0.81}$

Z反变换：$y(kT) = 1 - 0.5 \times 0.81^k$

\textbf{典型值}：
\begin{itemize}
    \item $y(0) = 0.5$ （初值）
    \item $y(10T) = 1 - 0.5 \times 0.81^{10} = 0.94$ （1秒后）
    \item $y(\infty) = 1$ （终值，稳态误差为0）
    \item 无超调（一阶系统）
\end{itemize}
...
\draw[->] (0,0) -- (6,0) node[right] {$t$ (秒)};
\draw[->] (0,0) -- (0,3) node[above] {$y(t)$};

% 参考线
\draw[dashed, gray] (0,2.5) -- (6,2.5) node[right] {$r=1$};

% 离散响应点
\foreach \k in {0,1,...,20} {
    \pgfmathsetmacro{\yval}{2.5*(1-0.5*0.81^\k)}
    \fill[blue] (0.3*\k, \yval) circle (1.5pt);
}

% 连接线（阶梯）
\draw[blue, thick] (0,1.25) -- (0.3,1.25) -- (0.3,1.59) -- (0.6,1.59) -- (0.6,1.86) -- (0.9,1.86) -- (0.9,2.06) -- (1.2,2.06) -- (1.2,2.22) -- (1.5,2.22) -- (1.5,2.34) -- (1.8,2.34) -- (1.8,2.43) -- (2.1,2.43) -- (2.1,2.49) -- (2.4,2.49);

\node[left] at (0,2.5) {1.0};
\node[left] at (0,1.25) {0.5};
\node[below] at (1.5,0) {1s};
\node[below] at (3,0) {2s};
\end{tikzpicture}
\end{center}

\subsection{数字控制器的设计与实现}

\textbf{设计目标}：为离散系统设计控制器$D(z)$，使闭环系统满足性能指标。

\textbf{常用方法}：
\begin{enumerate}
    \item \textbf{数字PID}：最常用，从连续PID离散化或直接设计（经验调参）
    \item \textbf{最少拍}：理论最优，在最少采样周期内达到期望输出（设计复杂）
    \item \textbf{数字串行校正}：类似连续系统超前/滞后校正
    \item \textbf{状态反馈}：现代控制方法（需要状态观测器）
\end{enumerate}

\textbf{实现方式}：差分方程 → C/Python代码 → 嵌入式系统运行

\subsubsection{设计方法一：数字PID控制器}
...
\textbf{【贯穿例子】为我们的系统设计数字PI控制器：}

目标：提高响应速度（减小调节时间）。使用位置式PI：
\begin{align*}
u(k) = K_p e(k) + K_i T \sum_{j=0}^k e(j)
\end{align*}

取$K_p=2$，$K_i=5$（经验值，实际需调试），$T=0.1$：
\begin{align*}
D(z) = K_p + K_i T \frac{z}{z-1} = 2 + 0.5\frac{z}{z-1} = \frac{2.5z - 2}{z-1}
\end{align*}

闭环传递函数（需验证稳定性）：极点需满足$|z_i|<1$

\subsubsection{设计方法二：最少拍控制}
...
\subsubsection{控制器的软件实现：从传递函数到代码}

\textbf{核心步骤}：传递函数$D(z)$ → 差分方程 → 可执行代码

\textbf{方法1：直接从传递函数推导差分方程}

设控制器传递函数：
\begin{align*}
D(z) = \frac{U(z)}{E(z)} = \frac{b_0 + b_1 z^{-1} + b_2 z^{-2}}{1 + a_1 z^{-1} + a_2 z^{-2}}
\end{align*}

交叉相乘：
\begin{align*}
U(z)[1 + a_1 z^{-1} + a_2 z^{-2}] &= E(z)[b_0 + b_1 z^{-1} + b_2 z^{-2}] \\
U(z) + a_1 z^{-1}U(z) + a_2 z^{-2}U(z) &= b_0 E(z) + b_1 z^{-1}E(z) + b_2 z^{-2}E(z)
\end{align*}
...
\begin{align*}
\text{sum} &= \text{sum} + e(k) \\
u(k) &= K_p e(k) + K_i T \cdot \text{sum} + K_d [e(k) - e(k-1)]/T
\end{align*}

\textbf{C语言实现示例（位置式PID）：}

\begin{lstlisting}[language=C, basicstyle=\small\ttfamily]
// 全局变量
float Kp = 2.0, Ki = 5.0, Kd = 0.0;  // PID参数
float T = 0.1;                         // 采样周期(秒)
float sum = 0.0;                       // 积分累积
float e_prev = 0.0;                    // 上一次误差

// 每个采样周期调用一次
float PID_Controller(float setpoint, float measurement) {
    float e = setpoint - measurement;  // 当前误差
    
    // 积分项累加
    sum += e;
    
    // PID计算
    float u = Kp * e + Ki * T * sum + Kd * (e - e_prev) / T;
...
class DigitalPID:
    ...
    def update(self, setpoint, measurement):
        ...
        self.sum += e
        u = self.Kp * e + self.Ki * self.T * self.sum + \
            self.Kd * (e - self.e_prev) / self.T
        self.e_prev = e
        return np.clip(u, -10, 10)  # 输出限幅

# 使用
pid = DigitalPID(Kp=2.0, Ki=5.0, Kd=0.0, T=0.1)
control_signal = pid.update(setpoint=1.0, measurement=0.5)
\end{lstlisting}

\textbf{【贯穿例子】我们的PI控制器实现：}

$D(z) = \frac{2.5z - 2}{z-1}$ 转差分方程：
\begin{align*}
U(z)(z-1) &= E(z)(2.5z - 2) \\
zU(z) - U(z) &= 2.5zE(z) - 2E(z) \\
u(k+1) - u(k) &= 2.5e(k+1) - 2e(k)
\end{align*}

递推形式：$u(k) = u(k-1) + 2.5e(k) - 2e(k-1)$

\begin{lstlisting}[language=C, basicstyle=\small\ttfamily]
float u_prev = 0.0, e_prev = 0.0;

float PI_Controller(float e) {
    float u = u_prev + 2.5*e - 2.0*e_prev;
    u_prev = u;
    e_prev = e;
    return u;
}
...
\begin{lstlisting}[language=C, basicstyle=\small\ttfamily]
// 主循环（每10ms执行一次，定时器中断触发）
void Timer_ISR(void) {
    float setpoint = 1.0;
    float measurement = ADC_Read();  // 从ADC读取传感器值
    
    float control = PID_Controller(setpoint, measurement);
    
    PWM_Set(control);  // 设置PWM占空比
}
\end{lstlisting}

\textbf{关键实现要点：}
\begin{itemize}
    \item \textbf{定时精度}：必须严格按$T$周期执行（用硬件定时器，不要用软件延迟）
    \item \textbf{数值精度}：用\texttt{float}（32位）通常够用，高精度系统用\texttt{double}
    \item \textbf{初始化}：所有历史变量（\texttt{sum}, \texttt{e\_prev}）初始化为0
    \item \textbf{抗饱和}：积分项和输出都需要限幅
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{第\ref{sec:lyapunov-stability}章（李雅普诺夫稳定性）}：
    \begin{itemize}
        \item 第\ref{subsec:discrete-lyapunov}节专门讲解\textbf{离散李雅普诺夫方法}
        \item 稳定性判据从左半平面改为单位圆内，但\textbf{本质}（能量衰减）不变
        \item 离散李雅普诺夫方程 $A^TPA-P=-Q$ 是本章稳定性分析的理论基础
        \item 包含完整的状态空间形式稳定性判断例题
    \end{itemize}
    \item \textbf{第\ref{sec:controllability-observability}章（能控能观性）}：判据在离散系统中\textbf{完全适用}（只需替换$A,B,C$）
\end{itemize}
