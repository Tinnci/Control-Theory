...
\textbf{常见误区：}
\begin{itemize}
    \item ✗ 认为能控就一定能观（两者独立，需分别判断）
    \item ✗ 只计算 $\text{rank}(B)$ 而不是 $\text{rank}(W_c)$
    \item ✗ 忘记能控性和能观测性是系统固有性质，与控制器无关
    \item ✓ 在设计阶段就考虑能控性和能观测性（执行器和传感器配置）
\end{itemize}

\textbf{后续章节预告：}
\begin{itemize}
    \item \textbf{结构分解}：将系统分解为能控/不能控、能观/不能观部分
    \item \textbf{极点配置}：利用能控性任意配置闭环极点
    \item \textbf{状态观测器}：利用能观测性估计不可测状态
\end{itemize}
