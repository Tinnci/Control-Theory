...
\subsection{奈奎斯特图绘制示例}
使用 \texttt{bodeplot} 包的 Nyquist 图功能：
...
\addplot[blue, thick, domain=0:2*pi, samples=100] 
    ({0.5*cos(deg(x)) - 0.5}, {0.5*sin(deg(x))});

% (-1,j0) 点
\addplot[mark=*, mark size=3pt, mark options={red}, only marks] 
    coordinates {(-1,0)};

% 标记
\node at (axis cs:-1,-0.2) {$(-1,j0)$};
\node at (axis cs:0,1.5) {$\omega \to +\infty$};
\node at (axis cs:0,-1.5) {$\omega \to -\infty$};
