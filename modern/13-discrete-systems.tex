...
\textbf{多种判定方法：}
\begin{enumerate}
    \item \textbf{特征值法}：直接求特征值，判断模是否小于1（最简单）
    \item \textbf{劳斯判据}：通过双线性变换转换到w平面（下文详述）
    \item \textbf{Jury判据}：直接在z域判断（下文详述）
    \item \textbf{李雅普诺夫方法}：能量递减法，适用于状态空间形式（见第\ref{subsec:discrete-lyapunov}节）
\end{enumerate}

\textbf{重要提醒：}离散系统的稳定域是\textbf{单位圆内}，与连续系统的左半平面完全不同！关于离散系统李雅普诺夫稳定性分析的详细内容，请参见第\ref{sec:lyapunov-stability}章第\ref{subsec:discrete-lyapunov}节。
...
\begin{itemize}
    ...
    \item \textbf{第\ref{sec:stability-analysis}章（稳定性）}：稳定性判据从左半平面改为单位圆内，但\textbf{本质}（衰减vs发散）不变
