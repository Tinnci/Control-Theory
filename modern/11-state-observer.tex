...
fprintf('期望极点: '); disp(poles');
fprintf('实际极点: '); disp(designed_poles);
% 允许小误差（数值误差），但位置应接近
    \end{lstlisting}
...
plot(t, e); xlabel('时间 (s)'); ylabel('误差 e(t)');
title('观测器误差收敛曲线');
% 检查：是否在预期时间内收敛到 0？
    \end{lstlisting}
    ...
    \begin{lstlisting}[style=Matlab-editor]
% 在输出 y 上加入噪声
y_noisy = y + 0.01*randn(size(y));  % 1% 噪声
% 运行观测器，观察 x_hat 的抖动
% 如果 x_hat 抖动剧烈 -> L 过大，需重新选择极点
    \end{lstlisting}
    
    \item \textbf{鲁棒性测试（可选）}
    \begin{lstlisting}[style=Matlab-editor]
% 故意引入模型误差
A_actual = A * 1.1;  % 假设 A 有 10% 误差
% 用原始 L 运行观测器，检查是否仍能收敛
    \end{lstlisting}
\end{enumerate}
