...
\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black, title=\faExclamationTriangle\ \textbf{重要提醒}]
\begin{itemize}
...
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title=\faStarO\ \textbf{极点配置定理（Pole Placement Theorem）}]
...
\begin{tikzpicture}[>=stealth, thick, node distance=2cm]
    % 能控性保证
    \node[draw, rectangle, rounded corners, fill=blue!10, text width=4cm, align=center] (ctrl) at (0,2) {
        \textbf{能控性} \\[0.1cm]
        $W_c = [B\ AB\ \cdots\ A^{n-1}B]$ \\
        张成\emph{整个状态空间}
    };
    
    \node[draw, rectangle, rounded corners, fill=green!10, text width=4cm, align=center] (inf) at (0,0) {
        \textbf{影响力} \\[0.1cm]
        通过 $BK$ 可在 \\
        \emph{所有方向}上施加影响
    };
    
    \node[draw, rectangle, rounded corners, fill=orange!10, text width=4cm, align=center] (pole) at (0,-2) {
        \textbf{极点配置} \\[0.1cm]
        可将 $A$ 的特征值 \\
        \emph{推到任意位置}
    };
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!60!black, title=\faCogs\ \textbf{极点配置设计流程}]
...
\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black, title=\faExclamationTriangle\ \textbf{定理的局限性}]
\begin{itemize}
...
\begin{tcolorbox}[colback=orange!5!white, colframe=orange!75!black, title=\faStar\ \textbf{方法3：Ackermann公式（最简洁！）}]
...
\hline
\rowcolor{blue!20}
\textbf{特性} & \textbf{直接方法} & \textbf{变换方法} & \cellcolor{orange!30}\textbf{Ackermann公式} \\
\hline
\textbf{适用范围} & 低阶系统（$n \leq 3$） & 任意阶次 & \cellcolor{yellow!20}单输入系统 \\
\hline
\textbf{计算复杂度} & 中等\\（需展开行列式） & 较高\\（需变换矩阵$T$） & \cellcolor{green!20}\textcolor{green!60!black}{\textbf{低}}\\（一个公式） \\
\hline
\textbf{直观性} & \textcolor{blue!80!black}{\textbf{高}}\\（直接配置） & 中等 & 低\\（公式较抽象） \\
\hline
\textbf{数值稳定性} & 好 & 取决于 $T$ & 取决于 $W_c$ \\
\hline
\rowcolor{yellow!10}
\textbf{推荐场景} & \textit{手算} $n \leq 3$ & \textit{理论分析} & \cellcolor{orange!20}\textcolor{red!80!black}{\textbf{程序实现}} \\
\hline
\end{tabular}
\end{center}
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!60!black, title=\faCheckCircle\ \textbf{原则1：稳定性要求（基本要求）}]
\begin{itemize}
...
\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title=\faClock\ \textbf{原则2：响应速度要求}]
\begin{itemize}
...
\begin{tcolorbox}[colback=yellow!5!white, colframe=orange!75!black, title=\faLineChart\ \textbf{原则3：超调和振荡要求}]
\begin{itemize}
...
\begin{tcolorbox}[colback=purple!5!white, colframe=purple!75!black, title=\faCubes\ \textbf{原则4：高阶系统的极点配置}]
\begin{itemize}
...
\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black, title=\faExclamationTriangle\ \textbf{原则5：实际限制}]
\begin{itemize}
